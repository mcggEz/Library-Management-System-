...
if (isset($_POST['submit_button'])) {
    ...
    if (mysqli_stmt_num_rows($userStmt) > 0) {
        ...
        $borrowedQuery = "SELECT id FROM borrowed_books WHERE control_number = ? AND acc_number = ? AND date_book_returned IS NULL";
        $borrowedStmt = mysqli_prepare($db, $borrowedQuery);
        mysqli_stmt_bind_param($borrowedStmt, "ss", $controlNumber, $accNumber);
        mysqli_stmt_execute($borrowedStmt);
        mysqli_stmt_store_result($borrowedStmt);

        if (mysqli_stmt_num_rows($borrowedStmt) > 0) {
            // Update the borrowed_books table to set is_returned to 1
            $updateQuery = "UPDATE borrowed_books SET is_returned = 1, date_book_returned = NOW() WHERE control_number = ? AND acc_number = ?";
            $updateStmt = mysqli_prepare($db, $updateQuery);
            mysqli_stmt_bind_param($updateStmt, "ss", $controlNumber, $accNumber);
        ...
        <div class="select-menu-container">
            <div class="select-menu">
                <div class="select-btn">
                    <span class="sBtn-text">Return Book</span>
            ...
            <div class="user_input_form">
                <section class="attendance">
                    ...
                    <form class="user_form_post" method="post">
                        ...
                        <input class="control_number_input" type="date" name="date" id="date" required> -->
