...
if (isset($_POST['submit_button'])) {
    ...
    $checkQuery = "SELECT time_in FROM library_space WHERE control_number = '$controlNumber' AND DATE(time_in) = CURDATE()";
    $checkResult = mysqli_query($db, $checkQuery);
    if (mysqli_num_rows($checkResult) > 0) {
        $message = "User already passed the form today.";
        $msg = true;
    } else {
        // Query the user table to retrieve the user's information and check if the user is blocked
        $blockQuery = "SELECT user_access FROM user WHERE control_number = '$controlNumber'";
        $blockResult = mysqli_query($db, $blockQuery);

        if ($blockResult && mysqli_num_rows($blockResult) > 0) {
            $blockRow = mysqli_fetch_assoc($blockResult);
            $userAccess = $blockRow['user_access'];

            if ($userAccess == 1) {
                // User is not blocked, proceed with inserting the attendance record
                $query = "SELECT control_number, age, sex, school_organization FROM user WHERE control_number = '$controlNumber'";
                ...
                if ($result && mysqli_num_rows($result) > 0) {
                    ...
                    // Insert the attendance record into the library_space table
                    $insertQuery = "INSERT INTO library_space (control_number, age, sex, school_organization, time_in) 
                                    VALUES ('$controlNumber', '$userAge', '$userSex','$userOrganization', NOW())";
                    mysqli_query($db, $insertQuery);
                    $message = "Control Number was recorded for library space usage successfully.";
                    $msg = true;
                } else {
                ...
                $message = "User is blocked. Please check the control number.";
                $msg = true;
            }
        } else {
            $message = "User not found. Please check the control number.";
        ...
        <div class="select-menu-container">
            <div class="select-menu">
                <div class="select-btn">
                    <span class="sBtn-text">Library Space</span>
